/* Hawthorn hash table module. This hash table code is used for both symbol tables
...
{
    TableInitTable(&hc->StringTable, &hc->StringHashTable[0], STRING_TABLE_SIZE, TRUE);
    hc->StrEmpty = TableStrRegister(hc, "");
}
...
int TableSet(Hawthorn *hc, struct Table *Tbl, char *Key, struct Value *Val, const char *DeclFileName, int DeclLine, int DeclColumn)
{
    ...
    if (FoundEntry == NULL)
    {   /* add it to the table */
        struct TableEntry *NewEntry = VariableAlloc(hc, NULL, sizeof(struct TableEntry), Tbl->OnHeap);
...
char *TableSetIdentifier(Hawthorn *hc, struct Table *Tbl, const char *Ident, int IdentLen)
{
    ...
    {   /* add it to the table - we economise by not allocating the whole structure here */
        struct TableEntry *NewEntry = HeapAllocMem(hc, sizeof(struct TableEntry) - sizeof(union TableEntryPayload) + IdentLen + 1);
        if (NewEntry == NULL)
...
{
    return TableSetIdentifier(hc, &hc->StringTable, Str, Len);
}

char *TableStrRegister(Hawthorn *hc, const char *Str)
{
    return TableStrRegister2(hc, Str, strlen((char *)Str));
}
...
void TableStrFree(Hawthorn *hc)
{
    ...
    for (Count = 0; Count < hc->StringTable.Size; Count++)
    {
        for (Entry = hc->StringTable.HashTable[Count]; Entry != NULL; Entry = NextEntry)
        {
